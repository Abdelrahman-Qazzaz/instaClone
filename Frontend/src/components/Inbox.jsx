...
function Inbox() {
  ...
  // PE
  useEffect(() => {
    // handles opening the chat from the profile page.
    if (params["*"].split("/")[0] == "t") {
  ...
  async function setupForOpeningChatFromProfilePage() {
    ...
    const targetUser_id =
      chat.user1_id == user._id ? chat.user2_id : chat.user1_id;
    const targetUser = user.following.find(
      (target) => target._id == targetUser_id
    );
    ...
    >
        ...
        {user &&
          ...
          user.following.map((following) => (
            <UserTab
              ...
              onClick={() => openChat(following)}
            />
          ))}
      </div>
      {path ==
      "/direct/inbox/" /*mobile only, if no chat is open, display this*/ ? (
      ...
      <Routes>
        ...
        {targetUser ? (
          <Route
            path="/t/*"
            element={<Chat targetUser={targetUser} path={path} />}
          />
        ) : null}
      </Routes>
    </div>
