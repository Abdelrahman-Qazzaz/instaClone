...
function UserContent(props) {
  ...
  //userList
  return showFollowersList ? (
    <SuggestedPage
      userList={props.targetUser.followedBy}
      onClick={() => setShowFollowersList(false)}
    />
  ) : showFollowingList ? (
    <SuggestedPage
      userList={props.targetUser.following}
    ...
    <div className="mt-4" style={{ width: "", overflowY: "auto" }}>
      <div className="container" style={{ maxWidth: "100%" }}>
        <div className="row m-0 my-2 p-0 border-top border-bottom">
          <div className="col-4 d-flex flex-column align-items-center">
            <div className="fw-bold">{props.targetUser.posts_ids.length}</div>
            <div style={{ color: "#758694" }}>posts</div>
          </div>
          <button
          ...
          >
            <div className="fw-bold">
              {props.targetUser.followedBy_ids.length}
            </div>
            <div style={{ color: "#758694" }}>followers</div>
          </button>
          <button
          ...
          >
            <div className="fw-bold">
              {props.targetUser.following_ids.length}
            </div>
            <div style={{ color: "#758694" }}>following</div>
          </button>
        </div>
        <div className="row m-0 p-0" style={{}}>
          <div className="col-4 d-flex mt-2 justify-content-center">
            <button
              onClick={() => {
                navigate("/" + baseProfilePagePath + "/");
              }}
              className="p-0"
              style={{
                border: "none",
          ...
          <div className="col-4 d-flex mt-2 justify-content-center">
            <button
              onClick={() => {
                navigate("/" + baseProfilePagePath + "/" + "reels/");
              }}
              className="p-0"
              style={{
                border: "none",
      ...
      <div className="mt-3">
        <Routes>
          <Route
            path={`/${baseProfilePagePath}/`}
            element={<PostsTab targetUser={props.targetUser} />}
          />
          <Route
            path={`/${baseProfilePagePath}/reels`}
            element={<ReelsTab targetUser={props.targetUser} />}
          />
        </Routes>
      </div>
    </div>
