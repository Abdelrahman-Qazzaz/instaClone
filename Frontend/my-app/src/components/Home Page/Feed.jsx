...
function Feed() {
  ...
  function updateFeedPostsArrayState(updatedPost) {
    const filteredFeedPosts = feedPosts.filter(
      (post) => post._id != updatedPost._id
    );
    const sorted = [...filteredFeedPosts, updatedPost].sort(
        ...
        >
          ...
          >
            ...
            {user.following.map((following) =>
              following.story &&
              following.story.slides &&
              following.story.slides.length &&
              following.story.slides.filter((slide) => {
                const watched =
                  slide.views &&
                  slide.views.find((view) => view.user_id == user._id)
                    ? true
                    : false;
                if (!watched) {
                  return slide;
            ...
            >
              {suggestedUsers && suggestedUsers.length ? (
                ...
                >
                  ...
                  >
                    <text
                      className="mx-md-0 mx-1"
                      style={{ fontSize: "1.1rem", fontWeight: "570" }}
                    >
                      Suggestions for you
                    </text>{" "}
                    <button
                      onClick={() => setShowSuggestedPage(true)}
                      style={{ fontWeight: "500" }}
                      className="btn text-info p-0 m-0 mx-1 mx-md-0"
                    >
                      See all
                    </button>
                  </div>
              ...
              {feedPosts && feedPosts.length
                ? feedPosts.map((feedPost) => {
                    return (
                      <FeedPost
                        post={feedPost}
                        updateFeedPostsArrayState={updateFeedPostsArrayState}
                      />
                    );
                  })
                : null}
