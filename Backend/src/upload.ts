    ...
    if (req.user._id == req.params.user_id) {
      try {
        upload.single("file")(req, res, async (err) => {
          ...
          const { text, textColor, textPositionX, textPositionY, duration } =
            req.body;
          const targetUser = await Users.findOne(filter);
          const prevSlides =
            targetUser.story && targetUser.story.slides
              ? targetUser.story.slides
              : [];
          const update = {
            $set: {
              story: {
                slides: [
                  ...prevSlides,
                  {
                    id: Date.now() + "",
                    firebasePathURL: url,
                    type: uploadedFile.mimetype.startsWith("video")
                      ? "video"
                      : "image",
                    text,
  ...
  try {
    upload.array("files")(req, res, async (err) => {
      ...
      for (const uploadedFile of uploadedFiles) {
        ...
        const type = uploadedFile.mimetype.startsWith("video")
          ? "video"
          : "image";
      ...
      if (req.route.path == "/chats/:chat_id/messages") {
        ...
        const filter = {
          _id: ObjectId.createFromHexString(req.params.chat_id),
        };
        const update = {
          $push: {
            messages: { sender_id: req.user._id, firebasePathURLs: urls, text },
          },
        };
        ...
        res.status(200).json({
          newMessage: {
            ...
            firebasePathURLs: urls,
            text,
          },
        });
      } else if (req.route.path == "/:user_id/posts") {
        ...
        await Users.updateOne(filter, update);
        res.status(201).json({
          posts_ids: [...owner.posts_ids, insertedId],
          newPost: {
            owner,
            firebasePathURLs: urls,
            caption: text,
...
async function uploadFileToFirebase(
  ...
  try {
    ...
    let update;
    console.log(reqRoutePath);
    if (reqRoutePath == "/:id/profile-picture") {
      update = { $set: { pfpFirebasePathURL: url[0] } };
      await Users.updateOne(filter, update);
    } else if (reqRoutePath == "/:user_id/posts") {
      const Posts = db.collection("Posts");
      const caption = reqBody.caption ? reqBody.caption : "";

      const Users = db.collection("Users");
      const owner = await Users.findOne({
        _id: ObjectId.createFromHexString(reqUser_id),
      });
      const result = await Posts.insertOne({
        owner,
        firebasePathURL: url[0],
        caption: caption,
        creationDate: Date.now(),
      });
